...
# Start a new Chrome/Chromium window (rather than just switch to the already open one)
browser_launch_cmd=$(command -v google-chrome || command -v chromium || true)
...
if [ -n "$browser_launch_cmd" ]; then
  gsettings set org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/custom3/ command "${browser_launch_cmd} --new-window"
else
  gsettings reset org.gnome.settings-daemon.plugins.media-keys.custom-keybinding:/org/gnome/settings-daemon/plugins/media-keys/custom-keybindings/custom3/ command
fi
